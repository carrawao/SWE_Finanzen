...
function changeData(view, data, labels, options, setup, datasetIndex) {
  if (labels[0]) {
    ...
    // must specify which dataset should be cut that's why we have datasetIndex
    setup.datasets[datasetIndex].data = data.slice(0, startDateIndex + 1);
    setup.labels = labels.slice(0, startDateIndex + 1);
  }
}

export const PortfolioValueChart = props => {
  ChartJS.register(...registerables);
  //console.log(props.portfolioData[props.activePortfolio].dailyDataForValueDevelopment);
  const dailyValueData = props.portfolioData[props.activePortfolio].dailyDataForValueDevelopment ? props.portfolioData[props.activePortfolio].dailyDataForValueDevelopment : {};
  ...
  return (
    <Card className='col-xl-12' raised sx={{border: 3, borderColor: 'rgb(73, 63, 53)', borderRadius: 3}}>
      <CardHeader title='Value Development'/>
      <CardContent>
