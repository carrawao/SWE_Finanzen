...
const SearchAssetInput = props => {
  ...
  const updateSearchAssetOptions = async (value) => {
    if (props.values.assetType === 'share') {
      ...
      return await fetchShareOptions(value);
    }
    if (props.values.assetType === 'crypto') {
      if (value === '') {
        return cryptoInPortfolioOptions;
      }
      return await fetchCryptoOptions(value);
    }
  }

  const searchCash = (searchText, options) => {
    return options.filter(option => {
      return option.symbol.toLowerCase().includes(searchText.toLowerCase()) || option.name.toLowerCase().includes(searchText.toLowerCase());
    });
  }
